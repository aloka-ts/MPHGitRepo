...
public class MphTestAppAinImpl implements ServiceInterface {

	/*
	 * lnpServiceImplObj : LnpServiceImpl instance to make this class singleton
	 */
	private static final MphTestAppAinImpl mphTestAppImplObj = new MphTestAppAinImpl();

	private static Logger logger = Logger.getLogger(MphTestAppAinImpl.class);

	/**
	 * @return LnpServiceImpl instance
	 */
	public static MphTestAppAinImpl getInstance() {
		return mphTestAppImplObj;
	}

	public static MphTestAppAinImpl getInstance(AseAlarmUtil aseAlarmUtilRef) {
		return mphTestAppImplObj;
	}
	 ...
	 * <p>
	 * To send 302 to orig:- 
	 * 1. Create ACTION with ACTION_REDIRECT 
	 * 2. Set LEG in ACTIOn to LEG1 which is orig leg 
	 * 3. Set Destination phone number PhoneNumber in orig LegData 
	 * 4. Set Protocol type in Action Refer method sendRedirect
	 * <p>
	...
	public Action[] processEvent(Event event, CallData callData) {
		...
		if (event != null && callData != null) {
			...
			case SIP:
				...
				break;
			case AIN_SCF:
				actionArr = processAinCS1Event(event, callData);
				break;
			default:
	 ...
	 * @return
	 */
	private Action[] processAinCS1Event(Event event, CallData callData) {
		String origLegCallId = (String) callData
				.get(CallDataAttribute.P_ORIG_LEG_CALL_ID);
		Action[] actionArr = null;
		logger.debug(origLegCallId + ":: Inside processEvent for Ain");
		...
		case EVENT_INITIAL: {
			...
			if (calledParty != null && calledParty.equals("9185471234")) {
				actionArr = connectAinTerm(origLegCallId, callData,
						Action.SEND_MODE.CONTINUE); // it is for connect flow
			} else if (calledParty != null && calledParty.equals("9185472345")) {
				actionArr = connectAinOrigIVR(origLegCallId, callData);// its is
			}
			else if (calledParty != null && calledParty.equals("9185473456")){
				actionArr = dropCall(origLegCallId, callData, null,
						Protocol.AIN_SCF);//for dropcall
			}
			else{
				actionArr = connectAinTerm(origLegCallId, callData,
		...
		case EVENT_PLAY_SUCCESS: {
			...
			if (CallDataAttribute.P_LEG1.equals(event.getLeg())) {
				...
				if (callData
					...
					logger.debug(origLegCallId
							+ "Ann played to orig. Now disconnect Ain IVR connection ");
	 ...
	 */
	public Action[] connectAinTerm(String origLegCallId, CallData callData,
	 ...
	 * return CONNECT_TERM 3. mPH will send CONNECT request to term
	 */
	public Action[] connectAinOrigIVR(String origLegCallId, CallData callData) {
		logger.debug(origLegCallId
				+ " connectTerm() Send initial INVITE to term and connect mPH to term");
		/*
		 * Dest Number which will be set in Contact header of 302 response
		...
		SccpUserAddress localAddr = AinScfProtocolConfig.getInstance()
