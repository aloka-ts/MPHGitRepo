...
public class MphTestAppInapCs1Impl implements ServiceInterface {

	/*
	 * lnpServiceImplObj : LnpServiceImpl instance to make this class singleton
	 */
	private static final MphTestAppInapCs1Impl mphTestAppImplObj = new MphTestAppInapCs1Impl();

	private static Logger logger = Logger.getLogger(MphTestAppInapCs1Impl.class);

	/**
	 * @return LnpServiceImpl instance
	 */
	public static MphTestAppInapCs1Impl getInstance() {
		return mphTestAppImplObj;
	}

	public static MphTestAppInapCs1Impl getInstance(AseAlarmUtil aseAlarmUtilRef) {
		return mphTestAppImplObj;
	}
	...
	Action[] processInapCS1Event(Event event, CallData callData) {
		...
		case EVENT_INITIAL: {
            ...
            String calledParty =(String)leg1Data.get(LegDataAttributes.P_CALLED_PARTY);
            
            logger.debug(origLegCallId + " Received a new call for protocol "
					+ event.getProtocol()+ " with calledParty "+calledParty);
			
			if (calledParty != null && calledParty.equals("1111")) {
			    actionArr = connectInapTerm(origLegCallId, callData,Action.SEND_MODE.END); // it is for connect flow
			}else if (calledParty != null && calledParty.equals("2222")){
				actionArr = connectInapTerm(origLegCallId, callData,Action.SEND_MODE.CONTINUE); // it is for connect flow
			}else if (calledParty != null && calledParty.equals("3333")){
		...
		case EVENT_DISCONNECT: {
			logger.debug(origLegCallId + " Received disconnect event "
					+ event.getProtocol());
