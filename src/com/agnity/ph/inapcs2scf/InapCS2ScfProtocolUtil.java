 ...
 * specific processing.
 */
public class InapCS2ScfProtocolUtil {

	private static Logger logger = Logger.getLogger(InapCS2ScfProtocolUtil.class);
	...
	static {
		...
		String protcolVersionList = InapCS2ScfProtocolConfig.getConfigData(InapCS2ScfProtocolConfig.ITUT_SEND_PROTOCOL_VER);
	...
	public static void startTimer(TcapSession tcapSession, long timeInMillies, boolean persistable, String timerName) {
		...
		phTimerInfo.setProtocol(Protocol.ITUINAPCS2_SCF);
	...
	private static Set<Action.ERB_TYPE> getErbSetInConfiguration() {
		...
		String configuredERBSet = InapCS2ScfProtocolConfig.getConfigData(InapCS2ScfProtocolConfig.ERB_MODE_AND_LEG_INFO);
	...
	private static void writeServiceCdr(CDR cdr, CallData callData) throws CDRWriteFailedException {
		...
		if (cdr != null && cdrArr != null) {
			...
			cdr.write(cdrArr);
			//  callData.set(CallDataAttribute.P_FINAL_CDR_WRITTEN, PhConstants.TRUE);// service will set this if it ahs already written cDR in failure case
		}
	...
	public static long getActivityTestTime(CallData callData) {
		...
		LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG1);
		Object atTimerDuration = legData.get(LegDataAttributes.P_AT_EXPIRY_TIME);
		long atTime = 30000;
		if (atTimerDuration != null) {
			atTime = (Long) atTimerDuration;
		}
		if (logger.isDebugEnabled()) {
			logger.debug("[PH]:: getActivityTestTime: " + atTime);
		}
		return atTime;
	}
	...
	public static void cleanupCorrelationResources(TcapSession tcapSession) {
		try {
			...
			if (corrIdObj != null && !(corrIdObj.toString().isEmpty())) {
				...
				InapCS2ScfProtocolUtil.returnTokenToSharedPool(callData);
			}
		} catch (Exception e) {
	...
	public static void startMaxCallDurationTimer(int dialogId,
		...
		if (maxDuration == null) {
			String mxDur = InapCS2ScfProtocolConfig
					.getConfigData(InapCS2ScfProtocolConfig.MAX_CALL_DURATION);
			if (mxDur != null) {
	...
	public static void returnTokenToSharedPool(CallData callData) {
		...
		String sharedPoolEnabled = InapCS2ScfProtocolConfig
	...
	public static Set<SS7ErbModeAndLegInfo> getErbSetByApplicationWithMode(CallData callData, String legID) {
		...
		String erbModeAndLegInfo = InapCS2ScfProtocolConfig
				.getConfigData(InapCS2ScfProtocolConfig.ERB_MODE_AND_LEG_INFO);
		if (logger.isDebugEnabled()) {
