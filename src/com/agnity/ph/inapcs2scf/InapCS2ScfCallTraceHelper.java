 ...
 */
public class InapCS2ScfCallTraceHelper {

	private static Logger logger = Logger.getLogger(InapCS2ScfCallTraceHelper.class);
			...
			if (traceMsg.length() > 0) {
				...
				if (callData.get(CallDataAttribute.P_TRACE_CONSTRAINT_ID) != null) {
					...
					for (int constraint : constraintList) {
						PhUtilityServices.getInstance((String)callData.get(CallDataAttribute.SERVICE_ID)).getCallTraceService().trace(constraint, InapCS2ScfProtocolUtil.getUniqueCallIDForTracing(callData), traceMsg.toString(), callState);
					}
				}
			...
			if (traceFlag.equals(PhConstants.TRUE)) {
				...
				if (callData.get(CallDataAttribute.P_TRACE_CONSTRAINT_ID) != null && dumpTrace) {
					...
					while (constraintIterator.hasNext()) {
						cCallTraceService.trace(constraintIterator.next(), InapCS2ScfProtocolUtil.getUniqueCallIDForTracing(callData), traceMsg.toString(), callState);
					}
				}
		...
		try {
			...
			if (PhConstants.TRUE.equals(isTraceFlag)) {
				...
				if (callData.get(CallDataAttribute.P_TRACE_CONSTRAINT_ID) != null) {
					...
					Iterator<Integer> constraintIterator = constraintList.iterator();
					while (constraintIterator.hasNext()) {
						cCallTraceService.trace(constraintIterator.next(), InapCS2ScfProtocolUtil.getUniqueCallIDForTracing(callData), traceMsg.toString(), callState,caller,called);
					}
				}
	...
	static void traceComponent(ComponentIndEvent cie, CallData callData) {
		try {
			...
			if (PhConstants.TRUE.equals(isTraceFlag)) {
				...
				PhoneNumber callingParty = null;
				if (leg1Data.get(LegDataAttributes.P_CALLING_PARTY) != null) {
					callingParty = (PhoneNumber) leg1Data
							.get(LegDataAttributes.P_CALLING_PARTY);
					caller = callingParty.getAddress();
				}

				PhoneNumber calledParty = (PhoneNumber) leg1Data
