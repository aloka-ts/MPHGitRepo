...
public class CC1Query {


	private static Logger logger = Logger.getLogger(CC1Query.class);
	 ...
	 * @throws Exception
	 */
	public static void decodeCc1Query(CallData callData, byte[] input)
			throws AINCodecException, InvalidInputException, ParameterOutOfRangeException {
		if (logger.isDebugEnabled()) {
			logger.debug("decodeCc1Query:Enter::" + Util.formatBytes(input));
		}

		LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG1);
		legData.set(LegDataAttributes.P_LIDB_QUERY_TYPE, LidBQueryType.CC1.name());

		if (logger.isDebugEnabled()) {
			logger.debug("decodeBuffer:set P_LIDB_QUERY_TYPE == CC1");
		}
		...
		if (input[2] != (byte) 0xDF || input[3] != (byte) 0x70) {
			logger.error("decodeBuffer:CC1 info id is not correct at pos2 & 3 " + CommonUtils.formatBytes(input));
			AinScfProtocolUtil.setApplicationErrorProblemData(callData, input, currentIndex, currentElementIdLen, currentElementLen);
			throw new AINCodecException("CC1 info id is not correct");
		}
		// operation to perform to set--->CC1 Info ID

		// check for CC1 Info length. - Row S
		if (input[4] != (byte) 0x00) {
			logger.error("decodeBuffer:CC1 info length should be zero at pos4. " + CommonUtils.formatBytes(input));
			AinScfProtocolUtil.setApplicationErrorProblemData(callData, input, currentIndex, currentElementIdLen, currentElementLen);
			throw new AINCodecException("CC1 info length should be zero");
		}
		// operation to perform on--->CC1 Info length.
		...
		if (input[9] == (byte) 0x05 && input[10] == (byte) 0x00 &&  input[11] == (byte) 0x11 &&  input[12] == (byte) 0x0A) {
			logger.info("decodeBuffer:Digits[Billing Number] type is CCAN with 10 digit length"
					+ CommonUtils.formatBytes(input));
			legData.set(LegDataAttributes.CC1_BILLING_NUM_TYPE, 0); //CCAN - This will be used only if ACG component is encoded, to set the numbering plan for ACG controlledCode.
			...
			logger.info("decodeBuffer:Digits[Billing Number] type is ACCAN with digit length "
					+ input[12]);
			legData.set(LegDataAttributes.CC1_BILLING_NUM_TYPE, 1); //ACCAN - This will be used only if ACG component is encoded, to set the numbering plan for ACG controlledCode.
		...
		ainDigitForBillingNum.setAddrSignal(decodeAdrsSignalForCc1(inputforDecodeAin, 0, 0));
		PhoneNumber bilingNumber = AinScfProtocolParser.parseAinDigits(ainDigitForBillingNum, natureOfBns);
		legData.set(LegDataAttributes.CC1_BILLING_NUM, bilingNumber);
		...
		if (input[newIndex] != (byte) 0xDF && input[newIndex + 1] != (byte) 0x60) {
			logger.error("decodeBuffer:PIN Id should be correct as pos " +newIndex
					+ CommonUtils.formatBytes(input));
			AinScfProtocolUtil.setApplicationErrorProblemData(callData, input, currentIndex, currentElementIdLen, currentElementLen);
			throw new AINCodecException("PIN  Identifier should be correct.");
		}
		newIndex = newIndex +2;
		...
		// Creating format for AIN digit decoder
		int arraySizePin = 2; // 2 octet BCD format 4 digits
		...
		String cC1PinNum = decodeAdrsSignalForCc1(inputforDecodeAinPin, 0, 0);
		legData.set(LegDataAttributes.CC1_PIN, cC1PinNum);
		newIndex = newIndex +2;
		...
		aindigitcalling.setAddrSignal(decodeAdrsSignalForCc1(inputfrDecodeAinclg, 0, 0));
		...
		if (input.length > calldPtyIndex ) {
			...
			aindigitcalled.setAddrSignal(decodeAdrsSignalForCc1(inputfrDecodeAincld, 0, 0));
		...
		if (logger.isDebugEnabled()) {
			logger.debug("CC1 decode:Exit");
		}
	}

	/**
	 * method to encode the CC1 Query
	 * 
	 * @param callData
	 * @return byte[]
	 * @throws InvalidInputException, AINCodecException
	 */
	public static byte[] encodeCc1Query(CallData callData) throws InvalidInputException, AINCodecException {
		if (logger.isDebugEnabled()) {
			logger.info("encodeCc1Query:Enter");
		}
		...
		if (leg2Data.get(LegDataAttributes.P_CC_CCAN_SERVICE_DENIAL_INDICATOR) != null) {
			logger.info("encodeCc1Query:parameter CSDI is present");
		...
		if (leg2Data.get(LegDataAttributes.P_CC_PIN_SERVICE_DENIAL_INDICATOR) != null) {
			logger.info("encodeCc1Query:parameter PSDI is present");
		...
		//PIN will go with RETURN ERROR message only
		byte[] pin = null;
		if (leg2Data.get(LegDataAttributes.P_CC_PIN) != null) {
			logger.info("encodeCc1Query:parameter pin is present");
			...
			if (pin.length != 2) {
				logger.info("encodeCc1Query : pin length should be 2");
		...
		if (leg2Data.get(LegDataAttributes.P_CC_RECORD_STATUS_INDICATOR) != null) {
			logger.info("encodeCc1Query:parameter record status indicator is present");
			...
			if (encodedRecStatInd.length != 1) {
				logger.info("encodeCc1Query:throwing exception ");
				logger.error("encodeCc1Quer:parameter record status indicator length should be 1");
		...
		if (leg2Data.get(LegDataAttributes.P_CC_CCSAN) != null) {
			logger.info("encodeCc1Query:parameter ccsan is present");
		...
		if (leg2Data.get(LegDataAttributes.P_CC_RAO_DIGIT) != null) {
			...
			if (raoDigit != null && raoDigit.length() == 3) {
				digitIdentifier = encodeAdrsSignalForCc1(raoDigit);
		...
		if (leg2Data.get(LegDataAttributes.P_CC_IC_INDICATOR_APIC) != null
				&& leg2Data.get(LegDataAttributes.P_CC_IC_INDICATOR_PPIC) != null
				&& leg2Data.get(LegDataAttributes.P_CC_IC_INDICATOR_PINC) != null) {
			logger.info(
					"encodeCc1Query:parameter ic indicators (APIC, PPIC & PINC) are present");
		...
		if (primaryPreferedIcDigits != null) {
			encodedPrimaryPreferedIcDigits = encodeAdrsSignalForCc1(primaryPreferedIcDigits.getAddress());
		...
		if (altPrefIcDigits != null) {
			encodedAltPrefIcDigits = encodeAdrsSignalForCc1(altPrefIcDigits.getAddress());
		...
		if (prefIncDigits != null) {
			encodedPrefIncDigits = encodeAdrsSignalForCc1(prefIncDigits.getAddress());
			encodedPrefIncDigitsNumDigit = (byte) encodedPrefIncDigits.length;
			totalLength += BnsConstants.BNS_PREF_INC_LEN;
		}
		if (trueBillNum != null) {
			encodedTrueNum = encodeAdrsSignalForCc1(trueBillNum.getAddress());
		...
		if(pin != null) {
			outputArray[index++] = CcConstants.PIN_TAG1;
			outputArray[index++] = CcConstants.PIN_TAG2;
			outputArray[index++] = CcConstants.CC_PIN_LEN;
	 ...
	 * @return a String
	 */
	private static String decodeAdrsSignalForCc1(byte[] data, int offset, int parity) throws InvalidInputException {
		if (logger.isDebugEnabled()) {
	 ...
	 * @return a byte array
	 */
	private static byte[] encodeAdrsSignalForCc1(String addrSignal) throws InvalidInputException {
	 ...
	 * @return byte[]
	 */
	public static byte[] getApplicationErrorForcc1(CallData callData) {
		...
		if(pin != null) {
			buffer[index++] = (byte) 0x05;
			buffer[index++] = CcConstants.PIN_TAG1;
			buffer[index++] = CcConstants.PIN_TAG2;
			buffer[index++] = CcConstants.CC_PIN_LEN;
			for (byte encodedVal : pin) {
				buffer[index++] = encodedVal;
			}
		}
		...
		if(logger.isDebugEnabled()){
			logger.debug("Application Error for CC1:" + CommonUtils.formatBytes(buffer));
		}
		return buffer;
	}
	 ...
	 * @return a byte array
	 */
	private static byte[] encodeCc1AINDigits(PhoneNumber ph) throws InvalidInputException {
	 ...
	 * @return byte []
	 */
	public static byte[] decimalToBcdForCc1(String input) {
		long num = 0;
		try {
	 ...
	 * @param callData
	 */
	public static void resetCC1(CallData callData) {
		if (logger.isDebugEnabled()) {
			logger.info("resetCC1:Enter");
		}
		...
		if(legData.get(LegDataAttributes.CC1_PIN) !=  null) {
			legData.remove(LegDataAttributes.CC1_PIN);
		}
		...
		if(leg2Data.get(LegDataAttributes.P_CC_PIN) != null) {
			leg2Data.remove(LegDataAttributes.P_CC_PIN);
		}
