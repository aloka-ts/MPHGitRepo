...
import com.agnity.ph.mapscf.MapScfProtocolUtil;
 ...
 */
public class InapCS1ScfProtocolHelper {

	private static Logger logger = Logger.getLogger(InapCS1ScfProtocolHelper.class);
	...
	static Action[] processIdp(InvokeIndEvent invokeIndEvent, TcapSession tcapSession) throws Exception {
		...
		// parse IDP
		try {
			InapCS1ScfProtocolParser.parseIdp(invokeIndEvent, callData);
		} catch (ASNParsingException ape) {
			logger.error("[PH]:: ASN pasring Exception in IDP.", ape);

			switch (ape.getParseFailType()) {
			case CPC_MISSING: {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_MISSING_CPC);
				break;
			}
			case TMR_MISSING: {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_MISSING_TMR);
				break;
			}
			case ACPC_MISSING: {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_MISSING_ACPC);
				break;
			}
			case FCI_MISSING: {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_MISSING_FCI);
				break;
			}
			default: {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_ASN_PARSING_FAIL);
				break;
			}
			}
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_INVALID_EXTN_TYPE);
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_PARAM_OUT_OF_RANGE);
		...
		/*
		 * checking if calling party is missing... commenting below as calling party is optional
		 */
		//        if ((callingPartyAddress == null || callingPartyAddress.isEmpty())) { //(contractorAddress == null || contractorAddress.isEmpty())
		//            logger.error("[PH]:: Calling party missing");
		//            callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.IDP_MISSING_CALLING_PARTY);
	...
	static Action[] processEnc(InvokeIndEvent invokeIndEvent, TcapSession tcapSession) throws Exception {
		...
		try {
			InapCS1ScfProtocolParser.parseEnc(invokeIndEvent, callData);
		} catch (ASNParsingException ape) {
			logger.error("[PH]:: ASN pasring Exception in ENC.", ape);
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ENC_ASN_PARSING_FAIL);
			return getASNParsingFailureAction(invokeIndEvent, tcapSession, ape);
		} catch (ParameterOutOfRangeException pore) {
			logger.error("[PH]:: ParameterOutOfRangeException in ENC.", pore);
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ENC_PARAM_OUT_OF_RANGE);
		...
		if (resultIndEvent.isLastResultEvent()) {
			...
			if (operCode != null) {
				...
				case InapOpCodes.ACTIVITY_TEST_BYTE: {
					...
					InapCS1ATHelper.cancelATTimer(tcapSession);
				}
				break;
				case InapOpCodes.PAC_BYTE: {
					InapCS1MediaServerHelper.parsePACResult(resultIndEvent, callData);
					Event event = new Event(EventType.EVENT_PNC_SUCCESS, Protocol.ITUINAPCS1_SCF, CallDataAttribute.P_LEG1.name());
					try {
				...
				default: {
					...
					callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.UNKNOWN_RESULT);
	...
	static Action[] processError(ErrorIndEvent errorIndEvent, TcapSession tcapSession) {
		...
		try {
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ERR_MSG_MAND_PARAM_MISSING);
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ERR_MSG_PARAM_NOT_SET);
			return getASNParsingFailureAction(tcapSession, MESSAGE.UERROR, FAILTYPE.DEFAULT);
		}

		int lastInvokeIdStart = InapCS1ScfProtocolUtil.getLastInvokeIdStartRange(callData);
		int lastInvokeIdEnd = InapCS1ScfProtocolUtil.getLastInvokeIdEndRange(callData);
		/*
		 * validateInvokeId
		 */
	...
	public static Action[] processReject(RejectIndEvent rejectIndEvent, TcapSession tcapSession) {
		...
		try {
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.REJECT_MSG_MAND_PARAM_MISSING);
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.REJECT_MSG_PARAM_NOT_SET);
			return getASNParsingFailureAction(tcapSession, MESSAGE.UREJECT, FAILTYPE.DEFAULT);
		}
		...
		if ((invokeId < lastInvokeIdStart) || (invokeId > lastInvokeIdEnd)) {
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.REJECT_INVALID_INVOK_ID);
		...
		if (problemType != ComponentConstants.PROBLEM_TYPE_INVOKE) {
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.REJECT_INVALID_PROBLEM_TYPE);
	 ...
	 * @return an array of Action Objects
	 */
	static Action[] getUnknownMessageAction(TcapSession tcapSession, InapCS1ScfProtocolEvent event) {
		if (logger.isDebugEnabled()) {
	 ...
	 * @return an array of Action Objects
	 */
	static Action[] getOutOfSequenceMsgAction(TcapSession tcapSession, InapCS1ScfProtocolEvent event) {
		CallData callData = InapCS1ScfProtocolUtil.getCallData(tcapSession);
		LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG1);
		if (logger.isDebugEnabled()) {
	 ...
	 * @return an array of Action Objects
	 */
	static Action[] getUnArmedErbEventAction(TcapSession tcapSession, InapCS1ScfProtocolEvent event) {
		if (logger.isDebugEnabled()) {
	...
	static Action[] getEntityReleasedAction(InvokeIndEvent invokeIndEvent, TcapSession tcapSession) {
		 ...
		 */
		try {
			InapCS1ScfProtocolParser.parseEntityRelease(invokeIndEvent, callData);
		} catch (Exception e) {
			 ...
			 * message
			 */
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ER_ASN_PARSING_FAIL);
	...
	static void sendErrorReqEvent(TcapSession tcapSession, Action action) throws Exception {
		...
		errorReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getLastRxInvokeId(callData));
	...
	public static void sendContinueRequestEvent(TcapSession tcapSession) throws Exception {
		...
		InapCS1ScfProtocolUtil.setLastInvokeIdStartRange(InapCS1ScfProtocolUtil.getLastInvokeIdEndRange(callData) + 1, callData);
		InapCS1ScfProtocolUtil.setLastInvokeIdEndRange(InapCS1ScfProtocolUtil.getLastInvokeId(callData), callData);
	}
	...
	private static void sendRejectReqEvent(TcapSession tcapSession, Action action) throws Exception {
		...
		rejectReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getLastRxInvokeId(callData));
	...
	static void sendEntityRelease(TcapSession tcapSession, Action action) throws Exception {
		...
		byte[] releaseCall = InapCS1ScfProtocolParser.createEntityRelease(callData, action);
		...
		erInvokeReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getNextInvokeId(callData));
	...
	public static void sendComponentReq(ComponentReqEvent cre, CallData callData) throws Exception {
		...
		InapCS1ScfCallTraceHelper.traceComponent(cre, callData);
	}
	...
	static void sendDialogueReq(DialogueReqEvent dre, CallData callData) throws Exception {
		...
		InapCS1ScfCallTraceHelper.traceDialog(dre, callData, PhUtilityServices.getInstance((String)callData.get(CallDataAttribute.SERVICE_ID)).getCallTraceService());
	}
	 ...
	 * @throws Exception
	 */
	public static void sendFci(TcapSession tcapSession) throws Exception {
		CallData callData = InapCS1ScfProtocolUtil.getCallData(tcapSession);
		if (logger.isDebugEnabled()) {
			logger.debug("[PH]:: Inside sendFci");
		}

		if( null != callData.get(CallDataAttribute.FCI)) {
			Boolean fciSentAlready = (Boolean) callData.get(CallDataAttribute.IS_FCI_SENT);
			if (null == fciSentAlready || !fciSentAlready) {
				byte[] fci = InapCS1ScfProtocolParser.createFci(callData);
				...
				fciInvokeReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getNextInvokeId(callData));
				...
				logger.debug("[PH]:: FCI has already been sent previously in this call.. Not sending again.");
			}
		}else{
			logger.debug("[PH]:: FCI is not set in call data.. Not sending..");
		}
	}
	...
	public static void connectTerm(TcapSession tcapSession, CallData callData, Action action) throws Exception {
		...
		if (action.getConnectionMode() == Action.CONNECTIONMODE.EQSROUTING && dialoutCheck) {
			...
			sendSipInviteFrmInapCS1(tcapSession, callData);
			return;
		}
		...
		case MS_DISCONNECTED:{
			if (action.getConnectionMode() == CONNECTIONMODE.REROUTING) {
				...
				// in case of outgoing dialogue the connectionMode = ReRouting and 
				// dialout flag is set to true.
				if(dialoutCheck){
					if(logger.isDebugEnabled()){
						logger.debug(dialogId + "Outgoing Dialogue on INAP");
					}
					InapCS1ScfOutgoingDialogue.initiateIDP(tcapSession, action);
				}else{
					if (logger.isDebugEnabled()) {
						logger.debug("[PH]:: Initiating Terminating Connection in REROUTING mode");
					}
					sendConnectTerm(tcapSession, action);
					InapCS1ScfProtocolUtil.updateSS7CallState(legData, InapCallStates.TERM_CONNECT_IN_PROGRESS);
					/*
					 * Set assist appsession timeout to 2 miniutes
					 */
					SipApplicationSession assistAppSession = InapCS1ScfProtocolUtil.getAssistAppSession(tcapSession,
				...
				if (PhConstants.TRUE.equals(sharedPoolEnabled)) {
					...
					correlationId = InapCS1ScfProtocolUtil.getTokenFromSharedPool(callData);
				...
				if (correlationId != null) {
					...
					InapCS1ScfProtocolUtil.updateSS7CallState(legData,
					...
					InapCS1ScfProtocolUtil
					.startTimer(tcapSession, InapCS1ScfProtocolUtil
					...
					//   callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.);
				...
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.INVALID_CONNMODE_TERM);
		...
		default: {
			...
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.UNEXP_ACT_CONNECT_TERM);
	...
	public static void dropCall(TcapSession tcapSession, CallData callData) {
		...
		case SERVICE_LOGIC:
			...
			action.setDropCallMode(DROP_CALL_MODE.USER_ABORT);
			action.setReleaseCauseValue(InapCS1ScfProtocolFSMHandler.AbortInfoEnum.NO_REASON_GIVEN.getCode());
			break;
		case HANDOFF:
			if (logger.isDebugEnabled()) {
		...
		try {
			...
			case ASSIST: {
				...
				if ((tcapSession.getAttribute(PhConstants.DFC_SENT) == null && 
					...
					InapCS1MediaServerHelper.sendDfc(tcapSession, action);
			 ...
			 */
			break;
			case MS_DISCONNECTED_END:
			case HANDOFF: {
				if (logger.isDebugEnabled()) {
					logger.debug("[PH]:: dropCall(session,action) HANDOFF or MS_DISCONNECTED_END, " +
							"resource cleanup on common action");
				}
				/*
	...
	public static void preProcessDroppedCall(TcapSession tcapSession) {
		...
		if (callData.get(CallDataAttribute.P_CORRELATION_ID) != null) {
			InapCS1ScfProtocolUtil.cleanupCorrelationResources(tcapSession);
		}
	 ...
	 * @throws Exception
	 */
	public static void sendEndRequestEvent(TcapSession tcapSession, boolean preArrangedEnd) throws Exception {
		CallData callData = InapCS1ScfProtocolUtil.getCallData(tcapSession);
		if (logger.isDebugEnabled()) {
		...
		if (leg2Data != null) {
			...
			SccpUserAddress destAddr = InapCS1ScfProtocolUtil.updateCalledAddress(callData, leg2Data);
			SccpUserAddress origAddr=	InapCS1ScfProtocolUtil.updateCallingAddress(callData, leg2Data);
			if(StringUtils.equalsIgnoreCase(CG_PTY_NO_PC,"true") || StringUtils.equalsIgnoreCase(CG_PTY_NO_PC,"1")) {
				if(logger.isDebugEnabled()) {
					logger.debug("Setting PtyNoPc to true in in orig addr endReqEvent1 in inap");
				}
				destAddr.setPtyNoPC(true);
			}else {
				if(logger.isDebugEnabled()) {
					logger.debug("Setting PtyNoPc to false in orig addr endReqEvent1 in inap");
				}
				destAddr.setPtyNoPC(false);
			}
			if(StringUtils.equalsIgnoreCase(CD_PTY_NO_PC,"true") || StringUtils.equalsIgnoreCase(CD_PTY_NO_PC,"1")) {
				if(logger.isDebugEnabled()) {
					logger.debug("Setting PtyNoPc to true in dest addr endReqEvent1 in inap");
				}
				origAddr.setPtyNoPC(true);
			}else {
				if(logger.isDebugEnabled()) {
					logger.debug("Setting PtyNoPc to false in dest addr endReqEvent1 in inap");
				}
	...
	static void sendReleaseCall(TcapSession tcapSession, Action action) throws Exception {
		...
		byte[] releaseCall = InapCS1ScfProtocolParser.createReleaseCall(callData, action);
		if (logger.isDebugEnabled()) {
		...
		rcInvokeReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getNextInvokeId(callData));
	...
	public static void notifyCallDropped(TcapSession tcapSession, boolean executeServiceAction) {
		try {
			...
			Event event = new Event(EventType.EVENT_CALL_DROPPED, Protocol.ITUINAPCS1_SCF, null);
	...
	private static void sendConnectTerm(TcapSession tcapSession, Action action) throws Exception {
		...
		if (PhConstants.TRUE.equals(fsmModel)) {
			if(logger.isDebugEnabled()){
				logger.debug("sendConnectTerm: FSM Model RRBCSM->FCI->CONNECT as seperate dialogue");
			}
			...
			// send Connect, whether to send as continue or end is decided based on 
			// send mode. 
			sendConnectAsDialogue(tcapSession, CONNECT_TYPE.TERMINATING, action, 
					((action.getSendMode() == Action.SEND_MODE.END)?false:true));
		}else{
			if(action.getSendMode() != Action.SEND_MODE.END) {
				// check if FCI need to be send forcefully. If set then we need to toggle 
				if (action.isForcedFciFlag()) {
					// Set it false explictly so that FCI can be sent again. 
					if(null != callData.get(CallDataAttribute.IS_FCI_SENT)){
						callData.set(CallDataAttribute.IS_FCI_SENT, false);
					}
				...
				// Request Report BCSM Event
				if (InapCS1ScfProtocolUtil.getErbSetByApplication(callData, action.getLeg()) != null) {
					InapCS1BCSMHelper.sendRRBCSMForArming(tcapSession, action, null);
				}
			}

			if (action.getSendMode() == Action.SEND_MODE.END) {
				callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.INAP_CALL_CONNECTED);
				preProcessDroppedCall(tcapSession);
		...
		legData.set(LegDataAttributes.P_FCI_BILLING_CHARACTERISTICS,null);
	}
	...
	private static void sendConnect(TcapSession tcapSession, CONNECT_TYPE connectType, Action action) throws Exception {
		...
		if (connectType == CONNECT_TYPE.TERMINATING) {
			connect = InapCS1ScfProtocolParser.createConnectForTerm(callData, action);
			if (logger.isDebugEnabled()) {
			...
			connect = InapCS1ScfProtocolParser.createConnectForPort(callData);
		} else if (connectType == CONNECT_TYPE.CORRELATION) {
			tcapSession.setAttribute(PhConstants.FOR_HANDOFF, PhConstants.TRUE);
			SccpUserAddress sua = (SccpUserAddress) legData.get(LegDataAttributes.P_SUA);
			connect = InapCS1ScfProtocolParser.createConnectForHandoff(callData, sua);
		} else {
	...
	public static void sendConnectHandoff(TcapSession tcapSession, Action action) throws Exception {
		...
		if (leg1TriggersArmed != null && leg1TriggersArmed.equals(PhConstants.TRUE)) {
			...
			InapCS1BCSMHelper.sendRRBCSMForDisarming(tcapSession, action);
		}
	...
	public static Action[] processAcr(InvokeIndEvent invokeIndEvent, TcapSession tcapSession) throws Exception {
		...
		// parse ACR
		try {
			InapCS1ScfProtocolParser.parseAcr(invokeIndEvent, callData);
		} catch (ASNParsingException ape) {
			logger.error("[PH]:: ASN pasring Exception in ACR.", ape);
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ACR_ASN_PARSING_FAIL);
			return getASNParsingFailureAction(invokeIndEvent, tcapSession, ape);
		} catch (ParameterOutOfRangeException pore) {
			logger.error("[PH]:: ParameterOutOfRangeException in ACR.", pore);
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ACR_PARAM_OUT_OF_RANGE);
	...
	public static void sendContinueComponent(TcapSession tcapSession, CallData callData, Action action) throws Exception {
		...
		byte[] cont = InapCS1ScfProtocolParser.createContinue(callData);
		...
		continueInvokeReqEvent.setInvokeId(InapCS1ScfProtocolUtil.getNextInvokeId(callData));
	...
	public static void sendResetTimer(TcapSession tcapSession, CallData callData, Action action) throws Exception {
		...
		byte[] ac = InapCS1ScfProtocolParser.createResetTimer(callData);
		byte[] acOpCode = { InapOpCodes.RESET_TIMER_BYTE };
		...
		resetInvokeReqEvent.setInvokeId(InapCS1ScfProtocolUtil
		...
		sendComponentReq(resetInvokeReqEvent, callData);
		sendContinueRequestEvent(tcapSession);
	}
	...
	public static Action[] processARI(InvokeIndEvent invokeIndEvent, TcapSession tcapSession) throws Exception {
		...
		// parse ARI
		try {
			InapCS1ScfProtocolParser.parseAri(invokeIndEvent, callData);
		} catch (ASNParsingException ape) {
			logger.error("[PH]:: ASN pasring Exception in ARI.", ape);
			callData.set(CallDataAttribute.NP_RELEASE_REASON_CODE, InapCS1ScfRelReasonCode.ARI_ASN_PARSING_FAIL);
			return getASNParsingFailureAction(invokeIndEvent, tcapSession, ape);
		} catch (ParameterOutOfRangeException pore) {
		 ...
		 */
		InapCS1ScfProtocolUtil.cleanupCorrelationResources(tcapSession);
		InapCS1MediaServerHelper.sendConnectToResource(tcapSession, callData, null);
		return null;
	}
	...
	private void startCdrTimer(TcapSession tcapSession, CallData callData) {
		...
		InapCS1ScfProtocolUtil.startTimer(tcapSession, initialDelay, false, PhConstants.CDR_TIMER);
	}
	...
	public static boolean invokeServiceChaining(TcapSession tcapSession,
		...
		Event event = new Event(EventType.EVENT_INITIAL,
				Protocol.ITUINAPCS1_SCF, CallDataAttribute.P_LEG1.name());
		...
		if (nextServiceId != null) {
			...
			InapCS1ScfProtocolUtil.resetNetworkTransactions(callData);
			...
			MeasurementCounter measurementCounter = PhMeasurementService.getInstance().getMeasurementCounter(Protocol.ITUINAPCS1_SCF);
			...
			if(action.isDropCallOnNoNextSvc()){
				...
				callData.set(
						...
						InapCS1ScfRelReasonCode.REJECT_NO_NEXT_SERVICE);
				/*
	...
	public static void incrementTcapCounters(EventObject eventObject) {
		MeasurementCounter measurementCounter = PhMeasurementService.getInstance().getMeasurementCounter(Protocol.ITUINAPCS1_SCF);
		if(measurementCounter == null) {
			return;
		}
	 ...
	 * @param callData
	 */
	public static void sendSipInviteFrmInapCS1(TcapSession tacpSession, CallData callData) 
			throws Exception  {
		if(logger.isDebugEnabled()){
			logger.debug("Enter sendSipInviteFrmInapCS1");
		}
		boolean outboundGwNotFound = true;

		try {
			createSipInviteFrmInapCS1(tacpSession, callData);
			outboundGwNotFound = false;

			if(logger.isDebugEnabled()){
				logger.debug("sendSipInviteFrmInapCS1: exit");
			}

		} catch (ServletParseException e) {
			logger.info("Exception inside sendSipInviteFrmInapCS1:: " + e);
		} catch (IOException e) {
			logger.info("Exception inside sendSipInviteFrmInapCS1:: " + e);
		...
		if(outboundGwNotFound) {
			...
			LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG2);
			legData.set(LegDataAttributes.P_CAUSE_CODE, InapCS1ScfRelReasonCode.OBGW_NOT_AVAILABLE);

			logger.error(tacpSession.getDialogueId() + ": OutboundGateway not found");
			Event event = new Event(EventType.EVENT_GW_NOT_FOUND, Protocol.ITUINAPCS1_SCF, legId);
	...
	private static void createSipInviteFrmInapCS1(TcapSession tcapsesion, CallData calldata)
			throws IOException, ServletException {
		if(logger.isDebugEnabled()){
			logger.debug("Inside createSipInviteFrmInapCS1::enter ");
		}
		...
		if(logger.isDebugEnabled()){
			logger.debug("LegData1 Inside createSipInviteFrmInapCS1 is:: " + legdata1);
		}

		LegData legdata2 = (LegData) calldata.get(CallDataAttribute.P_LEG2);
		if(logger.isDebugEnabled()){
			logger.debug("LegData2 Inside createSipInviteFrmInapCS1 is:: " + legdata2);
		}
		...
		if(logger.isDebugEnabled()){
			logger.debug("NP_GW_POOL_ID in createSipInviteFrmInapCS1::"+eqspoolId);
		}
		...
		sipRequest.send();
		if(logger.isDebugEnabled()){
			logger.debug("Inside createSipInviteFrmInapCS1::exit");
		}
	}
	...
	public static void startApplicationTimer(TcapSession appSession,
		...
		InapCS1ScfProtocolUtil.startTimer(appSession, action.getTimerTimeInMills(),
				false, timerName);

		ServiceInterface serviceHandler = PhUtilityServices
				.getInstance((String)callData.get(CallDataAttribute.SERVICE_ID)).getServiceHandler();
		Event appEvent = new Event(EventType.EVENT_APP_TIMER_STARTED, Protocol.ITUINAPCS1_SCF, action.getLeg());
	...
	public static void stopApplicationTimer(TcapSession appSession,
		...
		InapCS1ScfProtocolUtil.stopTimer(appSession, timerName);

		try{
			ServiceInterface serviceHandler = PhUtilityServices
					.getInstance((String)callData.get(CallDataAttribute.SERVICE_ID)).getServiceHandler();
			Event appEvent = new Event(EventType.EVENT_APP_TIMER_STOPPED, Protocol.ITUINAPCS1_SCF, action.getLeg());
	 ...
	 * @throws Exception
	 */
	public static void sendFciAsDialogue(TcapSession tcapSession) throws Exception {
		CallData callData = InapCS1ScfProtocolUtil.getCallData(tcapSession);
		LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG2);
		int dialogueId = (Integer) callData.get(CallDataAttribute.P_DIALOG_ID);

		if (logger.isDebugEnabled()) {
			logger.debug(dialogueId + "[PH]:: Inside sendFciAsDialogue");
		}

		String fciChrInfoStr = (String) legData.get(LegDataAttributes.P_FCI_BILLING_CHARACTERISTICS);

		if(fciChrInfoStr == null){
			if(logger.isDebugEnabled()){
				logger.debug(dialogueId 
						+ "[PH]:: sendFciAsDialogue - do not send FCI as P_FCI_BILLING_CHARACTERISTICS is null in Leg2");
			}
			return;
		}

		if (logger.isDebugEnabled()) {
			logger.debug(dialogueId + "[PH]:: Inside sendFciAsDialogue:: fci dump=" + fciChrInfoStr);
		}
	 ...
	 * @throws Exception
	 */
	private static void sendConnectAsDialogue(TcapSession tcapSession, CONNECT_TYPE connectType, 
			Action action, boolean sendAsContinue) throws Exception {

		CallData callData = InapCS1ScfProtocolUtil.getCallData(tcapSession);
		LegData legData = (LegData) callData.get(CallDataAttribute.P_LEG1);
		if (logger.isDebugEnabled()) {
			logger.debug("[PH]:: Inside sendConnectAsDialogue, connectType is: connectType is" +
					connectType.name() +", sendDialogue as Continue:" + sendAsContinue);
		}
		...
		if(logger.isDebugEnabled()){
			logger.debug("sendConnectAsDialogue: sending Connect:" + CommonUtils.formatBytes(connect) +
					" As " + ((sendAsContinue == true)?"Continue": "End"));
		}
	}
