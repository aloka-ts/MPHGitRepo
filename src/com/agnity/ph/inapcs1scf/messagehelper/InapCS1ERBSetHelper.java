 ...
 */
public class InapCS1ERBSetHelper {

	private static Logger logger = Logger.getLogger(InapCS1ERBSetHelper.class);
	...
	private static LinkedList<BCSMEvent> getConfiguredRRBSCMList(CallData callData, Action action) throws Exception {
		...
		Set<SS7ErbModeAndLegInfo> erbTypeSetObj = InapCS1ScfProtocolUtil.getErbSetByApplicationWithMode(callData, action.getLeg());
		...
		for (SS7ErbModeAndLegInfo erbTypeObj : erbTypeSetObj) {
			...
			case ERB_NO_ANSWER: {
				...
				InapCS1ERBSetHelper.updateBCSMEventForNoAnswer(bcsmEvent, noAnswerTimeObj);
				bcsmEventList.add(bcsmEvent);
			}
			break;
			case ERB_ANSWER: {
				BCSMEvent bcsmEvent = InapCS1ERBSetHelper.getBCSMEvent(
						...
						MonitorMode.EnumType.notifyAndContinue, erbTypeObj.getLegType()) ;
				bcsmEventList.add(bcsmEvent);
			}
			break;
			case ERB_DISCONNECT: {
	...
	public static BCSMEvent getBCSMEvent(EventTypeBCSM.EnumType enumType, MonitorMode.EnumType monitorModeType, 
			int legType) {
